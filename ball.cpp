...
Returns:
CollisionType	collosion in the vertical, horizontal, or none
*/
Ball::CollisionType Ball::collisionCheck(Block block)
{
	...
	if (distance < radius)
	{
		...
		// we are dividing the ball into 45 degrees quadrants with the upper and lower
		// resulting in a vertical collsion, and the left and right a horizontal collsion
...
void Ball::doCollision(int collisions)
{
	...
	if (collisions)
	{
		// for a collsion we invert the velocities and then move the ball its radius
			// in the new direction to get it out of the object it is colliding with, so we don't
			// end up with a 2nd collsion on the next update
