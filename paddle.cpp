...
/**
update location of paddle each frame relative to lag and perform collsionchecking with left
and right walls
Parameters: 
next		- user keyboard input
leftWall	- location of the left wall
right Wall	-location of the right wall
...
/**
draw the paddle with a border by drawing the block passed in on top
...
void Paddle::drawWithBorder(float lag, fgcugl::Color borderColor, int borderSize)
{
	...
	Block foreground = Block(x + borderSize, y + borderSize,
		width - 2 * borderSize, height - 2 * borderSize, color);
	foreground.draw();
